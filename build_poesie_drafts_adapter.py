...
SRC_ROOT = ROOT / "texte_drafts" / "poesie_drafts"     # Draft-Eingaben
DST_BASE = ROOT / "pdf_drafts" / "draft_poesie_pdf"    # Draft-Ausgaben
...
def run_one(input_path: Path) -> None:
    ...
    print(f"→ Erzeuge Draft-PDFs für: {input_path}")
    ...
    for name in new_pdfs:
        ...
        dst = target_dir / name
        src.replace(dst)
        print(f"✓ Draft-PDF → {dst}")

def main():
    if not RUNNER.exists():
        print(f"✗ {RUNNER.name} nicht gefunden – Abbruch."); sys.exit(1)
    if not SRC_ROOT.exists():
        print(f"✗ Draft-Eingabeordner fehlt: {SRC_ROOT} – Abbruch."); sys.exit(1)

    inputs = sorted(p for p in SRC_ROOT.glob("*/*.txt"))
    if not inputs:
        print(f"✗ Keine Draft-Eingaben in {SRC_ROOT}/<Autor>/*.txt – Abbruch."); sys.exit(1)
...
if __name__ == "__main__":
    main()
######## ENDE: build_poesie_drafts_adapter.py ########
