...
def _get_wortart_and_relevant_tags(token_tags: Set[str]) -> (Optional[str], Set[str]):
    ...
    # ABER: Kon wird NUR als Wortart erkannt, wenn es das EINZIGE Tag ist
    # (z.B. tribuendoqueAblKonGer soll als Verb erkannt werden, nicht als Kon)
    
    # Spezialfall: Wenn Kon vorhanden ist UND andere Tags, ignoriere Kon komplett
    if 'Kon' in token_tags and len(token_tags) > 1:
        # Prüfe andere Tags (ohne Kon) in WORTART_IDENTIFIER_TAGS
        for tag, wortart in WORTART_IDENTIFIER_TAGS.items():
            if tag != 'Kon' and tag in token_tags:
                return wortart, token_tags
        # Kein anderer Identifier gefunden, fahre mit komplexer Logik fort
        # (z.B. tribuendoqueAblKonGer hat Abl+Ger, die nicht in WORTART_IDENTIFIER_TAGS sind)
    elif 'Kon' not in token_tags:
        # Normale Prüfung (Kon ist nicht vorhanden)
        for tag, wortart in WORTART_IDENTIFIER_TAGS.items():
            if tag in token_tags:
                return wortart, token_tags
    # Wenn Kon das EINZIGE Tag ist, wird es unten als 'kon' erkannt
    ...
    # AUCH wenn zusätzlich Kon vorhanden ist (z.B. cogitandiqueGKonGer)
    if hat_lat_verbform:
        return 'verb', token_tags
    if hat_kasus and not hat_tempus:
        # Nomen: nur Kasus-Tag(s), AUCH mit Kon erlaubt (z.B. sollertiaqueAblKon)
        # Entferne Kon aus der Prüfung
        tags_ohne_kon = token_tags - {'Kon'}
        if tags_ohne_kon and all(t in KASUS_TAGS for t in tags_ohne_kon):
